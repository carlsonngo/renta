...
class User extends Authenticatable
{
    ...
    public function scopeSearch($query, $data = array(), $selects = array(), $queries = array()) {
        ...
        $query->select($select_data)->from('users');

        // Get data by site id
        $query->where('users.site_id', get_domain() );
        ...
        $query->where('users.id', '!=', 1); // Except super admin
    ...
    public function getGroupNameAttribute() {
        $post = Post::where('site_id', get_domain())->where('post_name', $this->group);
        if( $post->exists() ) 
