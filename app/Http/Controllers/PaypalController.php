...
class PaypalController extends Controller
{
    ...
    public function payOrder() {
        ...
        $currency = $this->setting->get_setting('currency');
        ...
        foreach ($cart['orders'] as $order) {
            ...
            $item = new Item();
            $item->setName($order['name'].' - sku: '.$order['sku']) 
        ...
        if( $cart['delivery_fee'] ) {
            $item = new Item();
            $item->setName('Delivery Fee')
                ->setCurrency($currency)
                ->setQuantity(1)
                ->setPrice($cart['delivery_fee']);
        ...
        $transaction->setAmount($amount)
            ->setItemList($item_list)
            ->setDescription("Payment for ordered by ".$inputs['email_address']);

        $redirect_urls = new RedirectUrls();
        $redirect_urls->setReturnUrl(URL::route('shop.place-order'))
            ->setCancelUrl(URL::route('shop.checkout'));
