...
<div class="float-right">
    ...
    <a href="{{ URL::route($view.'.index', query_vars()) }}" class="btn btn-sm btn-outline-dark"><i class="fa fa-long-arrow-left"></i> {{ trans('backend.all_galleries') }}</a>
...
<form method="POST">
    ...
    <div class="row">
        <div class="col-lg-8 col-md-7">
		    ...
		    <div class="row media-gallery sortable px-2">
		    <input type="hidden" name="gallery" value="">
		    @if( $gallery = json_decode(@$info->post_content, true) )
				...
				<div class="m-list col-lg-3 col-sm-6 mb-3 px-2">
					<div class="media-thumb img-thumbnail">
						<img src="{{ has_image(str_replace('large', 'medium', $slider_v)) }}" class="img-fluid w-100 mb-1">
						<input type="hidden" name="gallery[slider][{{ $slider_k }}]" value="{{ $slider_v }}">
						<input type="text" name="gallery[name][{{ $slider_k }}]" class="form-control rounded-0" value="{{ $gallery['name'][$slider_k] }}">
                        ...
                        Mark as FSK18</label>
						<a href="" class="delete-media" tabindex="-1"><i class="fas fa-trash"></i></a>
					</div>
				</div>
		        @endforeach
		    @endif
