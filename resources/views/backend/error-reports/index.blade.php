...
<table class="table table-hover table-striped border bg-white table-bordered">
    <thead>
        <tr>
            ...
            <th width="300">{{ trans('backend.message') }}</th>
            <th width="300">{{ trans('backend.domain_name') }}</th>
            <th>URL</th>
            <th class="text-center">{{ trans('backend.count') }}</th>
    ...
    <tbody>
        ...
        <tr class="has-actions">
            ...
            <td>
                <a href="{{ route($view.'.edit', $row->id) }}" class="text-dark">{{ str_limit($row->post_title, 100) }}</a>
                ...
                <div class="table-actions small text-uppercase mt-2">
                    ...
                    @else
                        | <a href="{{ URL::route($view.'.edit', $row->id) }}">{{ trans('backend.view') }}</a>
            ...
            <td>
	            <?php $url = parse_url($row->post_name); ?>
	            <a href="{{ $url['host'] }}" class="text-dark" target="_blank">{{ $url['host'] }}</a>
        	</td>
            <td><a href="{{ $row->post_name }}" target="_blank">{{ $row->post_name }}</a></td>
            <td class="align-middle text-center">{{ number_format($row->post_order) }}</td>
